...
opts_chunk$set(fig.path="figs/", fig.align="left")
options(width=80)
```
...
First, let's aggregate the load data we have been working with up to here from hourly to daily:
```{r aggregation}
daily.load<-ddply(SLDC, .(Date), numcolwise(mean))
```

Next, let's import temperature data corresponding to the same date range.
...
Based on the scatterplot (and knowledge of the system), we can assume that the temp-load correlation is driven by heating loads at cool temperatures up until ~65 deg. F (room temperature), at which there is little-to-no demand for thermal comfort energy services. At higher temperatures, we enter a cooling-regime where the temp-load correlation is driven mostly by air-conditioning and to a lesser extent, fans.  
...
abline(mod)
```
Alas, we can use the regression model to predict daily peak load given a daily mean temperature. This is the first step in creating an accurate load-forecast!
