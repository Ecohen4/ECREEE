...
Of course there are other weather-related determinants of energy demand besides temperature (including humidity and windspeed), as well as economic activity determinants such as weekday/weekend, holiday/non-holiday, school/no-school, etc.. However, the effect of these determinants are typically secondary to temperature, especially in extreme climates such as Delhi. 

As an example, let's try to quantify the temperature-load correlation for Delhi.  We can always add complexity to our model later by including additional load determinants.
...
# Alternatively, we could have writted the data to a new .csv file.
...
Given hourly demand data, we can study diurnal patterns, and importantly, how those patterns change throughout the year.  This provides insight into how and when energy is used, and thus what it is likely used for.  As a result, this information can be used for peak-load forecasting: Identify factors that drive demand (e.g. climate, weather, population, affluence and technology) and then apply projections of how those drivers are likely to change over time to predict demand. In this example, we are only looking at the effect of temperature on demand, but given the right data, we could apply these same techniques to test the effect of any explanatory variables (e.g. drivers of demand, as noted previously).
...
```{r}
# summarize
hourly.ptm<-ddply(SLDC, .(month, hours), summarize, 
      peak_to_mean=max(Demand.met)/mean(Demand.met), 
      peak_to_trough=max(Demand.met)/quantile(Demand.met,0.05)
      )
# plot [**PRETTIFY** x axis]
ggplot(hourly.ptm, aes(x=hours, y=peak_to_mean, group=month, colour=month)) +
  geom_line() +
  facet_wrap(~month) +
  scale_x_discrete(breaks=c("00","06","12","18","23")) + 
  labs(title="Characteristic diurnal peak-to-mean ratios, by month")

ggplot(hourly.ptm, aes(x=hours, y=peak_to_trough, group=month, colour=month)) + 
  geom_line() +
  facet_wrap(~month) +
  scale_x_discrete(breaks=c("00","06","12","18","23")) +
...
...and as a timeseries for each day of the year:
```{r peak-to-mean}
...
# remove outliers due to 2012 July 30/31 blackout
ggplot(subset(daily.ptm, Date != "2012-07-30" & Date != "2012-07-31"), aes(x=Date, y=peak_to_mean)) + geom_line()
ggplot(subset(daily.ptm, Date != "2012-07-30" & Date != "2012-07-31"), aes(x=Date, y=peak_to_trough)) + geom_line()
```
### Discussion:
...
Similarly, let's look at the peak-to-mean and peak-to-trough ratio over an entire month. What do you notice here?
```{r monthly}
# Summarize the data by month, calculating the min, mean and max 'Demand.met'.
ddply(diurnal, .(month), summarize, 
      peak_to_mean=max(mean)/mean(mean), 
      peak_to_trough=max(mean)/min(mean)
      )

monthly<-ddply(SLDC, .(month), summarize, 
               min=min(Demand.met), 
               mean=mean(Demand.met), 
               max=max(Demand.met), 
               sd=sd(Demand.met), 
               peak_to_mean=max(Demand.met)/mean(Demand.met), 
               peak_to_trough=max(Demand.met)/quantile(Demand.met,0.01)
               )
monthly
```
...
Now that we have coressponding temperature and load data for one year, we can build a simple linear regression model to empiricallly quantify the effect of temperature on load. To fit an ordinary least-squares model, we can use the `lm` function (short for linear model).  
Use ```?lm``` for details.
```{r temp-load-cor}
