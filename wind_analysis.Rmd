...
```{r read.data, cache=TRUE, results='hide'}
...
Subset data by type. For most of this analysis, we only need cumulative energy delivered and windspeed. We can compute the rest from first principles. Create a new `data.frame` called `dat` that contains just the information we want:
...
#dat<-subset(data, select=c("date_time", "mean_wind_mps", "min_wind_mps", "max_wind_mps", "cum_energy_delivered_kwh", )) # keep only windspeed and cumulative energy
```

Use the `summary` function for a statistical summary of the data contained in each column of the `data.frame` `dat`.
```{r summary}
summary(dat) # besides NAs, stats look OK at first glance
```
...
```
Now the summary statistics look more reasonable. Let's see what the data looks like now with respect to time.
...
```
Wind data looks OK besides the first bin. First bin is far too large. So we remove these values.
...
Plot the power curve, with benchmarks:
```{r benchmark.plot, fig.width=9, fig.height=5}
...
```{r filtered.benchmark.plot, ref.label="benchmark.plot", eval=FALSE}
```
...
### Fit a Weibull distribution to the statistically filtered windspeed data
...
### Plot meteorolgical windspeed vs. power output (curtailed and uncurtailed)
...
### Put it all together: Manufactures power curve, Uncurtailed Power Curve (with windspeed correction), Curtailed Power Curve (with windspeed correction).
...
Prediction
--------------------------------------
### Fit a local-polynomial power curve
