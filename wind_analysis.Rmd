...
opts_chunk$set(fig.path="figs/", echo=TRUE, cache=TRUE, fig.align="center")
...
```{r filtered.benchmark.plot, ref.label="benchmark.plot", fig.width=8, fig.height=5, warning=FALSE, eval=TRUE}
```
...
cf.simulation.uncurtailed.epc<-sum(expected.gen.empirical)/(850*7*8760)
cf.simulation.uncurtailed.mpc<-sum(expected.gen.manufacturer)/(850*7*8760)
...
# capacity factor
cf.expectation.uncurtailed.mpc<-expected.power.mpc/c
cf.expectation.uncurtailed.epc<-expected.power.epc/c

# compare uncurtailed capacity factor estimates from two techniques (simulation, expectation), and for two power curves (empricial and manufacturer)

# Simulation | Expectation 
# ----------------------------- | -----------------------------
# cf.simulation.uncurtailed.mpc | cf.expectation.uncurtailed.mpc
# cf.simulation.uncurtailed.epc | cf.expectation.uncurtailed.epc

cf<-data.frame(technique=c(rep("simulation",2), rep("expectation", 2)), 
               power.curve=c("manufacturer", "empirical", "manufacturer", "empirical"), 
               capacity.factor=c(cf.simulation.uncurtailed.mpc, cf.simulation.uncurtailed.epc, cf.expectation.uncurtailed.mpc, cf.expectation.uncurtailed.epc))
