...
for (i in 1:500){
  # Use a Weibull distribution to randomly generate one-year of 10-minute average windspeeds using the same shape and scale factor as the observed windspeeds
  windspeeds<-rweibull(8760*6, shape=weibull.fit$estimate[1], scale=weibull.fit$estimate[2])
#   if (i==1){expected.wind<-windspeeds} else
#     expected.wind<-rbind(expected.wind, windspeeds)  
  
  # estimate generation given new windspeeds
  gen.mpc<-predict(manufacturer.mod, newdata=windspeeds)
  if (i==1){expected.gen.mpc<-gen.mpc} else
    expected.gen.mpc<-rbind(expected.gen.mpc, gen.mpc)

  # estimate generation given new windspeeds
  gen.empirical<-predict(empirical.mod, newdata=windspeeds)
  if (i==1){expected.gen.empirical<-gen.empirical} else
    expected.gen.empirical<-rbind(expected.gen.empirical, gen.empirical)
  }
  
# compute annual generation from the hourly estimated generation. 1 annual value per simulation
ann.gen.mpc<-apply(expected.gen.mpc, 1, sum)
ann.gen.empirical<-apply(expected.gen.empirical, 1, sum)

# compute capacity factor in each year
cf.mpc<-ann.gen.mpc/(850*7*8760)
cf.empirical<-ann.gen.empirical/(850*7*8760)

# show the range of estimates for capacity factor given the ensemble forecast
boxplot(cf.mpc, main="Capacity Factor Simulation\n w. Weibull Distribution and the MPC", ylab="Capacity Factor")
boxplot(cf.empirical, main="Capacity Factor Simulation\n w. Weibull Distribution and the Empirical Power Curve", ylab="Capacity Factor")
```