...
opts_chunk$set(fig.path="figs/", echo=TRUE, cache=TRUE, fig.align="left")
...
Check if we **introduced NA, NaN or Inf** values in our calculations:
```{r introduced.NAs}
# check for NA
check(dat)
...
Next, check the energy sentout data. We can apply varying levels of stringency in filtering the data:
...
```{r write.csv}
write.csv(dat, file="filtered_VXE_wind_speed.csv")
save(dat, file="filtered_VXE_wind_speed.rdata")
```
...
denscomp(weibull.fit, addlegend=FALSE, main="Histogram and Weibull fit to lightly-treated windspeed data")
denscomp(weibull.fit.physics.filter, addlegend=FALSE, main="Histogram and Weibull fit to physics-filtered windspeed data")
denscomp(weibull.fit.stat.filter, addlegend=FALSE, main="Histogram and Weibull fit to statistically-filtered windspeed data")
denscomp(weibull.fit.corrected, addlegend=FALSE, main="Histogram and Weibull fit to statistically-filtered & corrected windspeed data")

# plot(weibull.fit, demp=TRUE, main="Weibull fit to lightly treated windspeed data")
# plot(weibull.fit.physics.filter, demp=TRUE, main="Weibull fit to physics-based filtered windspeed data")
# plot(weibull.fit.stat.filter, demp=TRUE, main="Weibull fit to statistically filtered windspeed data")
# plot(weibull.fit.corrected, demp=TRUE, main="Weibull fit to statistically filtered & corrected windspeed data")
```
...
expected.wind=rweibull(8760*6, shape=weibull.fit.stat.filter$estimate[1], scale=weibull.fit.stat.filter$estimate[2])
...
```{r p.forecast}
# discretize windspeed range
wind<-seq(0,24, by=0.1)

# vector of power output given a vector of windspeeds, according to the power curve
power.epc<-predict(empirical.mod, newdata=wind)
power.mpc<-predict(manufacturer.mod, newdata=wind)

# compute the probability of each windspeed according to the fitted weibull distribution
...
# expectation of power output
expected.power.mpc<-sum(pdf*power.mpc) #kw
expected.power.epc<-sum(pdf*power.epc) #kw
...
var.epc<-sum(pdf*(power.epc-expected.power.epc)^2)
sd.epc<-var.epc^(1/2)
list(expectation=expected.power.epc, variance=var.epc, standard.deviation=sd.epc)

# installed capacity
c<-850*7 #kw

# capacity factor
cf.mpc<-expected.power.mpc/c
cf.epc<-expected.power.epc/c
list(capacity.factor.mpc=round(cf.mpc, digits=2), capacity.factor.epc=round(cf.epc, digits=2))
```
...
```{r energy.timeseries.2, fig.width=9, fig.height=5, include=FALSE}
# # Now we can create time-series visualizations of the data:
# # summarize
